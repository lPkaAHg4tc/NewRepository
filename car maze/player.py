import pygame
#from pygame.examples.go_over_there import delta_time###不太理解这一行是干嘛的hhh

import math
##import config这条已经不用了
###
...
class Player(pygame.sprite.Sprite):
    def __init__(self,center_x,center_y,forward_angle):
        ...
        self.width=100
        self.height=50
        self.forward_angle=forward_angle##页面刚刚被加载出来时，角色的朝向
        ...
        self.last_time=pygame.time.get_ticks()#返回当前时刻的数值，要注意这里的单位是毫秒
        self.delta_time=0#表示相邻两帧之间的时间间隔

        self.move_velocity_limit=220
        self.move_velocity=0
        self.move_acc=600
        self.rotate_velocity_limit=140##转动速度的上限
    ...
    def move(self,direction=1):
        ...
        if direction==1 and abs(self.move_velocity)>50:
            self.rotate(direction)##只有有速度时才能转方向，所以这辆车应该不是仰望U9
        ...
        if direction == -1 and (self.move_velocity) > 50:
            self.rotate(direction)


    def crash(self):##判断如果撞墙了怎么办，并且进行处理
